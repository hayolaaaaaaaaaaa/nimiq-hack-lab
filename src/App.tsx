...
function App() {
  ...
  async function connect(){
    ...
    try {
      ...
      if(a){setWallet(a);getMe().then(profile=>{setXp(profile.xp)}).catch(()=>{})}
    } catch (e) {
  ...
  return <main className="site">
    ...
    <section id="feature" className="feature-section"><div className="section-label">01 <span>DAILY OPERATION</span></div><div className="feature-card"><div><div className="feature-icon"><img src="/logo/operator-mark.svg" alt="OPERATOR"/></div><small>TODAY'S CHALLENGE</small><h2>{dailyOperation ? games.find(game => game.id === dailyOperation.gameId)?.name.toUpperCase() : "LOADING"}</h2><p>{wallet ? "One verified attempt. Your score is replayed and ranked by the server." : "Connect your Nimiq wallet to enter today's verified operation."}</p></div><div className="feature-side"><div><small>REWARD</small><strong>{dailyOperation ? `${dailyOperation.rewardNim} NIM` : "—"}</strong></div><div><small>QUALIFY AT</small><strong>{dailyOperation ? dailyOperation.qualificationScore.toLocaleString() : "—"}</strong></div><button className="gold-btn compact" disabled={!dailyOperation} onClick={()=>wallet && dailyOperation ? launchGame(dailyOperation.gameId as GameId, "daily") : connect()}>{wallet ? "START OPERATION" : "CONNECT TO PLAY"} ↗</button></div></div></section>
