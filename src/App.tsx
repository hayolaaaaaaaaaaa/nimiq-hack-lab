...
import { createChallenge, getDaily, getDailyStatus, getLeaderboard, getMe, joinChallenge, logoutSession, requestDailyReward, startRun, submitChallenge, submitRun, trackEvent, type DailyOperation, type DailyStatus, type Run } from "./api";
...
function App() {
  ...
  const [challengeGame,setChallengeGame]=useState<GameId>("block-rush");
  ...
  async function createFriendChallenge(){
    ...
    try { const challenge=await createChallenge(challengeGame); setChallengeToken(challenge.token); setChallengeMessage(`Share token ${challenge.token} with your friend`); setActiveRun({runId:"",seed:challenge.seed,gameId:challenge.gameId,mode:"ranked",expiresAt:challenge.expiresAt,challengeToken:challenge.token}); eventsRef.current=[]; runStartedAt.current=performance.now(); setGame(challenge.gameId as GameId); }
    catch(error){setChallengeMessage(error instanceof Error ? error.message : "Could not create challenge")}
  }
  async function joinFriendChallenge(){
    setChallengeMessage(null);
    if(!wallet){setChallengeMessage("Connect your wallet to join a challenge");return}
    try { const challenge=await joinChallenge(challengeToken.trim()); setActiveRun({runId:"",seed:challenge.seed,gameId:challenge.gameId,mode:"ranked",expiresAt:"",challengeToken:challenge.token}); eventsRef.current=[]; runStartedAt.current=performance.now(); setGame(challenge.gameId as GameId); }
    catch(error){setChallengeMessage(error instanceof Error ? error.message : "Could not join challenge")}
  }
  ...
  return <main className="site">
    ...
    <section className="feature-section"><div className="section-label">03 <span>FRIEND CHALLENGE</span></div><div className="feature-card"><div><small>SHARED SEED · NO MONEY BETTING</small><h2>CHALLENGE AN OPERATOR</h2><p>{challengeMessage || "Create a deterministic challenge, share its token, and compare server-verified scores."}</p></div><div className="feature-side"><select value={challengeGame} onChange={event=>setChallengeGame(event.target.value as GameId)}>{rankedGames.map(id=><option key={id} value={id}>{games.find(game=>game.id===id)?.name}</option>)}</select><button className="gold-btn compact" onClick={createFriendChallenge}>CREATE CHALLENGE ↗</button><input value={challengeToken} onChange={event=>setChallengeToken(event.target.value)} placeholder="PASTE TOKEN" aria-label="Challenge token"/><button className="gold-btn compact" onClick={joinFriendChallenge}>JOIN CHALLENGE ↗</button></div></div></section>
